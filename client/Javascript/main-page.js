...
const fetchMovies = async (page = 1, pageSize = 30) => {
  try {
    ...
    const response = await fetch(
      `${API_URL}/new?page=${page}&pageSize=${pageSize}`
    );
  ...
  visibleMovies.forEach((movie, index) => {
    ...
    movieCard.innerHTML = `
      ...
      movie.averageRating
            ...
            <p class="movie-card-big-genre">${genreSpans}</p>
        `;
...
function prevMovies() {
  currentIndex -= MOVIES_PER_PAGE;
  if (currentIndex < 0)
    currentIndex = Math.max(0, movies.length - MOVIES_PER_PAGE);
  renderMovies();
}
    ...
    movieCard.innerHTML = `
            <div class="movie-card-medium">
                <div class="movie-card-medium-image" style="background-image: url('${imageUrl}')">
                    <div class="rating ${getRatingClass(
                      movie.averageRating
                    )}">${movie.averageRating}</div>
                </div>
